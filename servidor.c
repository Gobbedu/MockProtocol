...
int main()
{
    ...
    // soquete = ConexaoRawSocket("eno1");      // desktop andersor c3
...
void get(unsigned char *buffer){
    ...
    unsigned char resposta_cli[TAM_PACOTE];
    ...
    if(!arquivo){        
        ...
        envia_msg(soquete, &serv_seq, ERRO, &flag, 1);
    ...
    envia_msg(soquete, &serv_seq, DESC_ARQ, mem, 16);
